...
(def-entity-type Judgement
  ...
  (f/required-entries
   ...
   (f/entry :observable c/observable-NEW)
   (f/entry :disposition c/DispositionNumber-NEW)
   (f/entry :disposition_name c/DispositionName-NEW)
   (f/entry :priority Priority)
   (f/entry :confidence (f/enum v/HighMedLow-NEW))
   (f/entry :severity Severity)
   (f/entry :valid_time c/ValidTime-NEW))
  (f/optional-entries
   (f/entry :reason f/any-str)
   (f/entry :reason_uri c/URI-NEW)
   (f/entry :indicators (f/seq-of rel/related-indicators-NEW))))
